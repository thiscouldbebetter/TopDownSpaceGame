...
class Demo {
    actions() {
        var accelerate = new Action("Accelerate", (universeWorldPlaceEntities) => {
            ...
            if (itemFuel.quantity >= fuelUsedByAcceleration) {
                ...
                actorLoc.accel.add(Coords.create().overwriteWith(actorOrientation.forward).multiplyScalar(.3 // accelerationPerTick
                ));
            }
        });
        var fire = new Action("Fire", (universeWorldPlaceEntities) => {
            ...
            if (itemFuel.quantity >= fuelConsumed) {
                ...
                var entityDefnProjectile = world.defn.entityDefnByName("Projectile");
                var entityToSpawn = entityDefnProjectile.clone().nameSet("[projectile]").propertyAddForPlace(new Locatable(actor.locatable().loc.clone()), null // place
                );
        ...
        var turnLeft = new Action("TurnLeft", (universeWorldPlaceEntities) => {
            ...
            var actorOrientation = actorLoc.orientation;
            var turnRate = .25;
            actorOrientation.forwardSet(actorOrientation.right.clone().multiplyScalar(0 - turnRate).add(actorOrientation.forward));
        });
        var turnRight = new Action("TurnRight", (universeWorldPlaceEntities) => {
            var actor = universeWorldPlaceEntities.entity;
            var actorLoc = actor.locatable().loc;
            var actorOrientation = actorLoc.orientation;
            var turnRate = .25;
            actorOrientation.forwardSet(actorOrientation.right.clone().multiplyScalar(turnRate).add(actorOrientation.forward));
        });
        var actions = [
            accelerate,
            fire,
    ...
    worldGrid(universe, sizeInStarsystems) {
        ...
        for (var y = 0; y < sizeInStarsystems.y; y++) {
            ...
            for (var x = 0; x < sizeInStarsystems.x; x++) {
                ...
                var starsystem = new Starsystem(starsystemName, starsystemDefn0.name, starsystemSizeInPixels, 
                // entities
                [
                    // background
                    entityDefnsByName.get("Background").clone().nameSet("Background").propertyAddForPlace(Locatable.fromPos(Coords.zeroes()), null // place
                    ),
                    // sun
                    entityDefnsByName.get("Sun").clone().nameSet("Sun").propertyAddForPlace(Locatable.fromPos(starsystemSizeInPixelsHalf.clone()), null // place
                    ).propertyAddForPlace(new Star(new NameGenerator().generateNameWithSyllables(3), null // color
                    ), null // place
                    ),
                    // portals
                    entityDefnsByName.get("PortalRed").clone().nameSet("PortalWest").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(.05 * starsystemSizeInPixels.x, starsystemSizeInPixelsHalf.y)), null // place
                    ).propertyAddForPlace(new Portal2(starsystemNamePrefix + starsystemPosWest.toString(), Coords.fromXY(.9 * starsystemSizeInPixels.x, starsystemSizeInPixelsHalf.y)), null),
                    entityDefnsByName.get("PortalGreen").clone().nameSet("PortalEast").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(.95 * starsystemSizeInPixels.x, starsystemSizeInPixelsHalf.y)), null // place
                    ).propertyAddForPlace(new Portal2(starsystemNamePrefix + starsystemPosEast.toString(), Coords.fromXY(.1 * starsystemSizeInPixels.x, starsystemSizeInPixelsHalf.y)), null // place
                    ),
                    entityDefnsByName.get("PortalBlue").clone().nameSet("PortalNorth").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(starsystemSizeInPixelsHalf.x, .05 * starsystemSizeInPixels.y)), null // place
                    ).propertyAddForPlace(new Portal2(starsystemNamePrefix + starsystemPosNorth.toString(), Coords.fromXY(starsystemSizeInPixelsHalf.x, .9 * starsystemSizeInPixels.y)), null // place
                    ),
                    entityDefnsByName.get("PortalViolet").clone().nameSet("PortalEast").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(starsystemSizeInPixelsHalf.x, .95 * starsystemSizeInPixels.y)), null // place
                    ).propertyAddForPlace(new Portal2(starsystemNamePrefix + starsystemPosSouth.toString(), Coords.fromXY(starsystemSizeInPixelsHalf.x, .1 * starsystemSizeInPixels.y)), null // place
                    ),
                ]);
                var numberOfPlanetsMin = 1;
                var numberOfPlanetsMax = 5;
                var numberOfPlanetsRange = numberOfPlanetsMax - numberOfPlanetsMin;
                var numberOfPlanets = numberOfPlanetsMin
                ...
                for (var p = 0; p < numberOfPlanets; p++) {
                    var pos = Coords.create().randomize(null).multiply(starsystemSizeInPixels).round();
                    var entityPlanet = entityDefnPlanet.clone().nameSet(Planet.name + p).propertyAddForPlace(Locatable.fromPos(pos), null // place
                    ).propertyAddForPlace(new Planet(nameGenerator.generateNameWithSyllables(2), Color.Instances().Orange), null);
                ...
                var numberOfEnemies = numberOfEnemiesMin
                ...
                for (var e = 0; e < numberOfEnemies; e++) {
                    var pos = Coords.create().randomize(null).multiply(starsystemSizeInPixels).round();
                    var entityEnemy = entityDefnEnemy.clone().nameSet("Enemy" + e).propertyAddForPlace(Locatable.fromPos(pos), null);
                ...
                for (var c = 0; c < numberOfItemCollections; c++) {
                    var pos = Coords.create().randomize(null).multiply(starsystemSizeInPixels).round();
                    var entityItemCollection = entityDefnItemCollection.clone().nameSet("ItemCollection" + c).propertyAddForPlace(Locatable.fromPos(pos), null);
                    starsystem.entitiesToSpawn.push(entityItemCollection);
                }
                if (starsystemPos.x == 0 && starsystemPos.y == 0) {
                    var entityPlayer = entityDefnsByName.get("Player").clone().nameSet("Player").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(100, 100)), null // place
                    );
                    starsystem.entitiesToSpawn.push(entityPlayer);
                    // friendlies
                    var entityFriendly = entityDefnsByName.get("Friendly").clone().nameSet("Friendly0").propertyAddForPlace(Locatable.fromPos(Coords.fromXY(350, 50)), null // place
                    );
        ...
        var defns = new WorldDefnExtended(colors, itemDefns, actions, activityDefns, entityDefns, starsystemDefns);
        var world = new WorldExtended("WorldGrid" + sizeInStarsystems.toString(), defns, starsystems);
        return world;
    }
    world_ItemDefns() {
        var returnValues = [
    ...
    world_ActivityDefns() {
        var doNothing = new ActivityDefn("DoNothing", 
        // perform
        (universeWorldPlaceEntities) => {
            // do nothing
        });
        var moveRandomly = new ActivityDefn("MoveRandomly", 
        // perform
        (universeWorldPlaceEntities) => {
            var world = universeWorldPlaceEntities.world;
            var actor = universeWorldPlaceEntities.entity;
            ...
            if (speedCurrent > 0) {
                var timeToTarget = distanceToTarget / speedCurrent;
                var moverDefn = MoverDefn.fromEntity(actor);
                var accelerationCurrent = moverDefn.force / moverDefn.massBase;
            ...
            if (distanceToTarget < distanceThreshold) {
                ...
                var forceToApplyTowardTarget = directionToAccelerate.clone().multiplyScalar(MoverDefn.fromEntity(actor).force);
                actorLoc.force.add(forceToApplyTowardTarget);
            }
        });
        var userInputAccept = new ActivityDefn("UserInputAccept", (universeWorldPlaceEntities) => {
            ...
            var actionToInputsMappingsByInputName = starsystemDefn.actionToInputsMappingsByInputName;
            for (var i = 0; i < inputsActive.length; i++) {
                var inputActiveName = inputsActive[i].name;
                if (inputActiveName.startsWith("Mouse") == false) {
                    var mapping = actionToInputsMappingsByInputName.get(inputActiveName);
                    if (mapping != null) {
    ...
    world_EntityDefns(universe) {
        ...
        var imagesForItemCollection = imageHelper.buildImagesFromStringArrays("ItemCollection", [
            [
                "aaaaa..",
                "a...aa.",
                "a...a.a",
        ...
        var imageMoverProjectile = imageHelper.buildImageFromStrings("Projectile", [
            "yyy",
            "y.y",
            "yyy",
        ]);
        ...
        */
        ...
        var imagesForPortal = imageHelper.buildImagesFromStringArrays("Portal", [
            [
                "...v.....v.....v...",
                "..v.v...v.v...v.v..",
                ".v...v.v...v.v...v.",
        ...
        var imagesFriendly = imageHelper.buildImagesFromStringArrays("Friendly", [
            [
                "......aaaaa......",
                ".....a.....a.....",
                "....a..ggg..a....",
        ...
        */
        ...
        var camera = new Camera(universe.display.sizesAvailable[0], // viewSize
        1, // focalLength
        Disposition.fromPos(Coords.zeroes()), null // entitiesInViewSort
        );
        var entityDefnCamera = new Entity("Camera", 
        // properties
        [
            new CameraDefn(camera),
        ]);
        var itemSizeInPixels = new Coords(10, 10, 1);
        var entityDefnItemCollection = new Entity("ItemCollection", 
        // properties
        [
            Animatable2.default(),
            ...
            [ticksPerAnimationFrame], VisualImageScaled.manyFromSizeAndVisuals(itemSizeInPixels, imagesForItemCollection.map(x => new VisualImageImmediate(x, false) // isScaled
            )), true // isRepeating
            ))),
            new ItemContainer(),
        ...
        var entityDefnProjectile = new Entity("Projectile", 
        // properties
        [
            new BodyDefn(projectileSizeInPixels),
            Collidable.from3(new Box(Coords.zeroes(), projectileSizeInPixels), [Enemy.name], (universeWorldPlaceEntities) => {
        ...
        var entityDefnPlanet = new Entity("Planet", [
            ...
            Collidable.from3(new Box(new Coords(0, 0, 0), planetSizeInPixels), [], function collide() { }),
        ...
        for (var c = 0; c < portalColorNames.length; c++) {
            ...
            var entityDefnPortal = new Entity(entityDefnName, [
                ...
                Drawable.fromVisual(new VisualCameraProjection(camera, new VisualAnimation(null, // name
                [ticksPerAnimationFrame], VisualImageScaled.manyFromSizeAndVisuals(portalSizeInPixels, imagesForPortal.map(x => new VisualImageImmediate(x, false)) // isScaled
                ), true // isRepeating
                ))),
        ...
        var imageBackgroundLayer0 = imageHelper.buildImageFromStrings("BackgroundLayer0", [
            "c..c..c",
            ".c.c.c.",
            "...c...",
            "ccccccc",
        ...
        var imageBackgroundLayer1 = imageHelper.buildImageFromStrings("BackgroundLayer1", [
            "..a..",
            "..a..",
            "aaaaa",
            "..a..",
        ...
        var backgroundCellSize = new Coords(backgroundViewSize.x * backgroundCellSizeMultiplier, backgroundViewSize.x * backgroundCellSizeMultiplier, // x, not y
        1);
        ...
        var entityDefnEnemy = new Entity("Enemy", [
            new Actor(Activity.fromDefnName("MoveRandomly")),
            Animatable2.default(),
            new BodyDefn(enemySize),
            Collidable.from3(new Box(new Coords(0, 0, 0), enemySize), [Player.name], 
            // collide
            ...
            VisualImageScaled.manyFromSizeAndVisuals(enemySize, imagesEnemy.map(x => new VisualImageImmediate(x, false)) // isScaled
            ), true // isRepeating
            ))),
            new Enemy(),
            Killable.fromIntegrityMax(1),
        ...
        var playerCollide = (universeWorldPlaceEntities) => {
            ...
            if (entityOther.propertyByName(ItemContainer.name) != null) {
                ...
                var itemTradeOffer = Planet.fromEntity(planet).itemTradeOffer;
                ...
                if (itemFuel.quantity >= fuelUsedByPortal) {
                    ...
                    var destinationStarsystem = world.starsystemByName(destinationStarsystemName);
        ...
        var entityDefnPlayer = new Entity("Player", [
            ...
            Collidable.from3(new Box(new Coords(0, 0, 0), playerSizeInPixels), [ItemContainer.name, Enemy.name, Planet.name, Portal2.name], playerCollide),
            Drawable.fromVisual(new VisualCameraProjection(camera, new VisualDirectional(new VisualNone(), VisualImageScaled.manyFromSizeAndVisuals(playerSizeInPixels, imagesForPlayerClockwise.map(x => new VisualImageImmediate(x, false) //, playerSizeInPixels)
    ...
    world_StarsystemDefns() {
        var starsystemDefns = [
            new StarsystemDefn("StarsystemDefn0", [
...
class Constraint_ConformToBounds {
    ...
    constrain(universeWorldPlaceEntities) {
        ...
        entityLoc.pos.trimToRangeMinMax(this.boxToConformTo.min(), this.boxToConformTo.max());
    }
    clone() { return this; }
...
class Constraint_FollowEntityByName {
    ...
    constrain(universeWorldPlaceEntities) {
        ...
        if (entityToFollow != null) // hack
         {
            entity.locatable().loc.pos.overwriteWith(entityToFollow.locatable().loc.pos);
        }
    }
    clone() { return this; }
    overwriteWith(other) { return this; }
