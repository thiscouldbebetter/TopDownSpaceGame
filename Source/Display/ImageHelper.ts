...
class ImageHelper
{
	...
	*/
	...
	buildImageFromStrings(name: string, stringsForPixels: string[]): any
	{
	...
	buildImagesFromStringArrays(name: string, stringArraysForImagePixels: string[][]): any[]
	{
		var returnValue = [];
	...
	buildImageFromStringsTiled
	(
		name: string, sizeInTiles: Coords, stringsForPixels: string[]
	): any
	{
	...
	canvasToImage(name: string, canvas: any): any
	{
	...
	imageToCanvasAndGraphicsContext(image: any): any[]
	{
		var canvas = document.createElement("canvas");
		var imageSizeInPixels = image.sizeInPixels;
	...
	overwriteColorWithPixelFromGraphicsAtPos
	(
		color: Color, graphics: any, pixelPos: Coords
	)
	{
		var pixelColorAsComponentsRGBA =
			graphics.getImageData(pixelPos.x, pixelPos.y, 1, 1).data;
	...
	sliceImageIntoTiles(imageToSlice: any, sizeInTiles: Coords): any[]
	{
		var returnImages = [];

		var systemImageToSlice = imageToSlice.systemImage;

		var imageToSliceSize = Coords.fromXY
		(
